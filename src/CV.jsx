...
class CV extends Component {
  ...
  componentDidMount() {
    axios.get("./src/data/cvdata.json").then((response) => {
      this.setState({
  ...
  render() {
    ...
    for (let i = 0; i < cvDatas.length; i++) {
      if (cvDatas[i].name == "Work") {
        cvWorkList.push(
          <div
            className="item"
            id={"work-" + cvDatas[i].id}
            key={cvDatas[i].id}
          >
        ...
        cvEduList.push(
          <div className="item" id={"edu-" + cvDatas[i].id} key={cvDatas[i].id}>
            <CVcard cvdata={cvDatas[i]} />
          </div>
        );
      }
      ...
      <React.Fragment>
        ...
        <div className="ui container">
          <div className="btngroup">
            <Link to="/contact">
              <button id="contactPage" className="ui big button">
                <i className="envelope icon"></i>Contact me
              </button>
            </Link>
          </div>
        </div>
